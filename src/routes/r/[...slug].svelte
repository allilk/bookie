	...
	onMount(async () => {
		...
		const getUser = async () => {
			...
			const resp = await fetch(`${localAPIURL}/api/profile/get`,{
				...
				headers: {
					'content-type': 'application/x-www-form-urlencoded',
					'Authorization': 'JWT eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InRlc3RAdGVzdC5jb20iLCJmdWxsTmFtZSI6InRlc3QiLCJfaWQiOiI2MDIwOGU3MzZjY2NmZTRkZGNmZjc4ZTIiLCJpYXQiOjE2MTI3NDY3Mjh9.dY2iCJju6_vzDKo5ULJ-W4XJNxg6W6CCWllOALSPbTE'
				},
				body: new URLSearchParams({
					userid: userId
				})
...
<RenderSingle promise={promise} fullName={fullName}></RenderSingle>